...
const listFoods = (recipe) => {
    ...
    for (let i = 0; i < ingredients.length; i++) {
        result.push(ingredients[i].slice(ingredients[i].indexOf(' ', (ingredients[i].indexOf(' ') + 1)) + 1));
    }
    return result;
  };
...
const splitFoods = (recipe) => {
  ...
  ingredients.forEach(value => {
    ...
    let removeItems = splitString.splice(0, 2);
    let sliced = splitString.slice(removeItems);
...
const extractVowels = (str) => {
  ...
  let noVow = str.replace(/[aeiou]/gi, '');

  for (let i = 0; i < vowelList.length; i++){
    let vowel = vowelList[i];
  ...
  vowelArr.sort()
  let vowelStr = vowelArr.join('');

  answerArr.push(noVow, vowelStr);
