...
const countNumberOfElements = (arr) => arr.reduce((accumulator, element) => accumulator + ((element) ? 1 : 0), 0)
...
const reversedString = (str) => {
  str = str.split('');
  return str.reduce((accumulator, element) => accumulator = element + accumulator);
};
...
const countNumberOfChildren = (arr) => {
  return arr.reduce((accumulator, element) => {
    if (element.children){
      accumulator += element.children.length;
    };
    return accumulator;
  }, 0);
};
...
const calculateAverage = (arr) => arr.reduce((accumulator, element) => accumulator += element)/arr.length;
...
const countPrimeNumbers = (arr) => arr.reduce((accumulator, element) => accumulator + ((isPrime(element)) ? 1 : 0), 0)
...
const extractChildren = (arr) => {
  ...
  let filterArr = arr.filter(element => element.name.includes('a'));
  console.log(filterArr);

  return filterArr.reduce((accumulator, element) => {
      if (element.children) {
        console.log('has kids!', element.name);
        accumulator = accumulator.concat(element.children);
      } 
      return accumulator;
    }, []) ;
};
